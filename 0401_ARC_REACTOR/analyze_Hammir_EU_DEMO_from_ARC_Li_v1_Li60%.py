...
# v2: ARC Li enrichment = 60%, EU-DEMO Li enrichment = 60%
#
# Input:
#   NOAK_ARC_resource_requirements_monthly.csv
#
# Note:
#   The input CSV column "li90_total_cum_t" is used directly.
#   In this modified scenario, it is interpreted as the
#   ARC-derived Li-60 supply.
#
#   No 0.90/0.60 scaling is applied.
#
# Output:
#   No CSV by default. Summary + plots only.
# ============================================================
...
SAVE_CSV = False
OUT_CSV = f"Combined_from_ARC_Li60_Be{BE_FIXED}_CAGR{CAGR_FIXED}.csv"
...
# ============================================================
# Lithium enrichment assumptions
# ============================================================

ARC_ENRICHMENT = 0.60
EUDEMO_ENRICHMENT = 0.60
HAMMIR_ENRICHMENT = 0.30
LI_NAT_ENRICHMENT = 0.075

LI60_TO_LI30_FACTOR = (
    (ARC_ENRICHMENT - LI_NAT_ENRICHMENT)
    / (HAMMIR_ENRICHMENT - LI_NAT_ENRICHMENT)
)
...
EUDEMO_LI60_STARTUP_T_PER_PLANT = 133.4
...
EUDEMO_LI60_OP_T_PER_PLANT_MONTH = (
    EUDEMO_LI6_BURN_KG_PER_YR
    / EUDEMO_ENRICHMENT
    / 1000.0
    / 12.0
)
...
# ============================================================
# ARC-derived Li60 supply
# ============================================================
# The original column name is li90_total_cum_t, but in this
# scenario it is interpreted directly as Li60 cumulative supply.
# No 0.90/0.60 scaling is applied.
# ============================================================

case["li60_supply_monthly_t"] = (
    case["li90_total_cum_t"].diff().fillna(case["li90_total_cum_t"])
)

case["li60_supply_cum_t"] = case["li60_supply_monthly_t"].cumsum()

# Hammir uses Li30 obtained by down-blending ARC-derived Li60
case["li30_supply_monthly_t"] = (
    case["li60_supply_monthly_t"] * LI60_TO_LI30_FACTOR
)

case["li30_supply_cum_t"] = case["li30_supply_monthly_t"].cumsum()

future_li60_monthly_supply = case["li60_supply_monthly_t"].values
...
def can_build_one_more_eudemo(
    ...
    current_count,
    current_month_index,
    future_li60_monthly_supply,
):
    test_inventory = inventory_now_t - EUDEMO_LI60_STARTUP_T_PER_PLANT
    ...
    for future_month in range(current_month_index + 1, len(future_li60_monthly_supply)):
        test_inventory += future_li60_monthly_supply[future_month]
        test_inventory -= test_count * EUDEMO_LI60_OP_T_PER_PLANT_MONTH
...
def human_format(x, pos):
    """
    Human-friendly y-axis formatting
    """
    if abs(x) >= 1e6:
        return f"{x/1e6:.1f}M"
    elif abs(x) >= 1e3:
...
e_inventory_li60_t = 0.0
e_n_built = 0
e_li60_startup_cum_t = 0.0
e_li60_operation_cum_t = 0.0
...
for idx, row in case.iterrows():
    ...
    li60_supply_monthly_t = float(row["li60_supply_monthly_t"])
    li60_supply_cum_t = float(row["li60_supply_cum_t"])
    ...
    # ========================================================
    # EU-DEMO deployment using Li60
    # ========================================================

    e_inventory_li60_t += li60_supply_monthly_t

    e_op_this_month_t = e_n_built * EUDEMO_LI60_OP_T_PER_PLANT_MONTH
    e_inventory_li60_t -= e_op_this_month_t
    e_li60_operation_cum_t += e_op_this_month_t

    if e_inventory_li60_t < 0 and abs(e_inventory_li60_t) < 1e-9:
        e_inventory_li60_t = 0.0

    if e_inventory_li60_t < -1e-9:
        raise RuntimeError(
            f"EU-DEMO Li60 inventory became negative at month {month}: "
            f"{e_inventory_li60_t:.6f} t"
        )

    e_new_builds = 0

    while e_inventory_li60_t >= EUDEMO_LI60_STARTUP_T_PER_PLANT:
        feasible = can_build_one_more_eudemo(
            inventory_now_t=e_inventory_li60_t,
            current_count=e_n_built + e_new_builds,
            current_month_index=idx,
            future_li60_monthly_supply=future_li60_monthly_supply,
        )

        if not feasible:
            break

        e_inventory_li60_t -= EUDEMO_LI60_STARTUP_T_PER_PLANT
        e_new_builds += 1

    e_startup_this_month_t = e_new_builds * EUDEMO_LI60_STARTUP_T_PER_PLANT
    e_li60_startup_cum_t += e_startup_this_month_t
    e_n_built += e_new_builds

    e_li60_total_used_t = e_li60_startup_cum_t + e_li60_operation_cum_t
    ...
    records.append({
        ...
        "arc_li60_supply_cum_t": li60_supply_cum_t,
        "arc_li60_supply_monthly_t": li60_supply_monthly_t,
        ...
        "eudemo_li60_total_used_t": e_li60_total_used_t,
        "eudemo_li60_inventory_remaining_t": e_inventory_li60_t,
    })
...
print(f"[INFO] ARC enrichment:       {ARC_ENRICHMENT:.2f}")
print(f"[INFO] EU-DEMO enrichment:   {EUDEMO_ENRICHMENT:.2f}")
print(f"[INFO] Hammir enrichment:    {HAMMIR_ENRICHMENT:.2f}")
print(f"[INFO] Li60 -> Li30 factor for Tandem Mirror: {LI60_TO_LI30_FACTOR:.4f}")
print("--------------------------------------------------")
...
print(f"[{NAME_ARC}] Li60 supply:   {last['arc_li60_supply_cum_t']:,.2f} t")
print("--------------------------------------------------")
...
print(f"[{NAME_EUDEMO}] Li60 used:     {last['eudemo_li60_total_used_t']:,.2f} t")
print(f"[{NAME_EUDEMO}] Li60 remain:   {last['eudemo_li60_inventory_remaining_t']:,.2f} t")
print("--------------------------------------------------")
if SAVE_CSV:
    print(f"[INFO] Wrote: {OUT_CSV}")
print("[INFO] Combined Li60 analysis complete.")
print("--------------------------------------------------")
...
# 4. Lithium inventories
# Uncomment if needed.
'''
plt.figure(figsize=(8, 5))
plt.plot(t, out["arc_li60_supply_cum_t"], color=COLOR_ARC, label="ARC-derived Li-60 supply")
plt.plot(t, out["eudemo_li60_total_used_t"], color=COLOR_EUDEMO, label="EU-DEMO Li-60 used")
plt.plot(t, out["li30_supply_cum_t"], color=COLOR_HAMMIR, linestyle="--", label="Li-30 supply for Hammir")
plt.plot(t, out["hammir_li30_total_used_t"], color=COLOR_HAMMIR, label="Hammir Li-30 used")
plt.xlabel("Time (year)")
plt.ylabel("Lithium Inventory / Use (t)")
plt.title("Lithium Supply and Use")
plt.grid(True, alpha=0.3)
plt.legend()
plt.tight_layout()
plt.gca().yaxis.set_major_formatter(FuncFormatter(human_format))
...
plt.plot(
    ...
    color=COLOR_HAMMIR,
    label="Remaining Li-30 inventory: Hammir",
)
plt.plot(
    t,
    out["eudemo_li60_inventory_remaining_t"],
    color=COLOR_EUDEMO,
    label="Remaining Li-60 inventory: EU-DEMO",
)
plt.xlabel("Time (year)")
plt.ylabel("Remaining Lithium Inventory (t)")
plt.title("Remaining Lithium Inventory")
plt.grid(True, alpha=0.3)
plt.legend()
plt.tight_layout()
plt.gca().yaxis.set_major_formatter(FuncFormatter(human_format))
...
plt.title(r"Nb$_3$Sn and Tungsten Boride (WB) Requirements")
plt.tight_layout()
plt.show()
